...
class IFNet(nn.Module):
    ...
    def align_images(
        ...
        def compute_flow(img0_blurred, img1_blurred, timestep):
            ...
            for i in range(4):
                if flow is None:
                    ...
                    wf0 = warp(f0, flow[:, :2], device, fp16)
                    wf1 = warp(f1, flow[:, 2:4], device, fp16)
                    fd, m0 = block[i](
                        torch.cat(
                            (
                                img0_blurred[:, :3],
        ...
        aligned_img0 = warp(img0, compensated_flow[:, :2], device, fp16)
    ...
    def forward(self, img0, img1, timestep=1, fastmode=True, ensemble=True, num_iterations=1, multiplier=0.5, blur_strength=0, device='cuda', fp16=True):
        ...
        for _iteration in range(num_iterations):
            aligned_img0, flow, flow2, img1_blurred = self.align_images(
                img0, img1, timestep, scale_list, blur_strength, ensemble, device, fp16
            , flow2, img1_blurred)
